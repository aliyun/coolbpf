		   ...
		   bool crosstask, bool add_mark)
{
	...
	if (kernel && !user_mode(regs)) {
		...
		printk("call perf callchain kernel\n");
		perf_callchain_kernel_p(entry, regs);
	}
	...
	if (user) {
		...
		if (regs) {
			...
			fs = get_fs();
			set_fs(USER_DS);
			printk("call perf callchain user\n");
...
int stack_map_init(void)
{
	int cpu;
	struct stack_map_irq_work *work;
	printk("init stack map\n");
