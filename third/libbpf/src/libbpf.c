...
static int bpf_object__sanitize_and_load_btf(struct bpf_object *obj)
{
	...
	if (bpf_drv_needed()) {
		pr_debug("bpf_drv module let it skip uploading btf\n");
		return 0;
	}
#endif
...
out_err:
	cp = libbpf_strerror_r(-err, errmsg, sizeof(errmsg));
	pr_warn("failed to pin map: %s, path: %s\n", cp, map->pin_path);
	return libbpf_err(err);
}
...
struct bpf_program *bpf_program_clone(const struct bpf_program *prog)
{
	struct bpf_program *clone_prog;
	clone_prog = malloc(sizeof(struct bpf_program));
	if (!clone_prog){
