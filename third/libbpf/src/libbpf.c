...
static int attach_legacy_kprobe(const char *name, uint64_t offset, bool retprobe,
		uint64_t *id)
{
	...
	snprintf(buf, sizeof(buf), "%c:kprobes/%s %s+%lu",
			retprobe ? 'r' : 'p', name, name, retprobe ? 0 : offset);
	...
	snprintf(buf, sizeof(buf), "/sys/kernel/debug/tracing/events/kprobes/%s/id",
			name);
...
static int detach_legacy_kprobe(const char *name)
{
	const char *file = "/sys/kernel/debug/tracing/kprobe_events";
	char buf[PATH_MAX];
	int fd, err = 0;
				 ...
				 uint64_t offset, int pid)
{
	...
	if (type < 0) {
		...
		pr_info("use legacy method to enable kprobe\n");

		err = attach_legacy_kprobe(name, offset, retprobe, &kprobe_id);
		if (err) {
			detach_legacy_kprobe(name);
			pr_warn("failed enable legacy kprobe: %s\n",
...
static int __bpf_link__detach_kprobe(struct bpf_link *link)
{
	int err;

	if (link->ctx) {
		char *name = (char *)link->ctx;

		// FIXME: handle error
		err = detach_legacy_kprobe(name);
	}
...
struct bpf_link *__bpf_program__attach_kprobe(struct bpf_program *prog,
						int pfd)
{
	struct bpf_link *link;
	...
	if (link)
		link->detach = &__bpf_link__detach_kprobe;
	return link;
}
					    ...
					    const char *func_name)
{
	...
	link = __bpf_program__attach_kprobe(prog, pfd);
	if (IS_ERR(link)) {
		close(pfd);
		err = PTR_ERR(link);
