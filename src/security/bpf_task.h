...
#ifndef SYSAK_BPF_TASK_H
#define SYSAK_BPF_TASK_H
...
FUNC_INLINE struct task_struct *get_parent(struct task_struct *t)
{
  ...
  bpf_probe_read(&task, sizeof(task), _(&t->real_parent));
  if (!task)
    return 0;
  return task;
}
...
#pragma unroll
  for (i = 0; i < TASK_PID_LOOP; i++) {
    ...
    bpf_probe_read(&cpid, sizeof(cpid), _(&task->tgid));
    if (cpid == pid) {
      i = TASK_PID_LOOP;
      continue;
    }
...
FUNC_INLINE __u32 get_task_pid_vnr(void)
{
  ...
  if (thread_pid_exists) {
    bpf_probe_read(&pid, sizeof(pid), _(&task->thread_pid));
    if (!pid)
      return 0;
  } else {
...
//     * try to run this bpf_probe_read and throw an error. So lets
//     * sanitize it for the verifier.
//     */
//    if (!thread_pid_exists)
//      link_sz =
//        24; // voodoo magic, hard-code 24 to init stack
//    bpf_probe_read(&link, link_sz,
//               (void *)_(&task->pids) + (PIDTYPE_PID * link_sz));
//    pid = link.pid;
  }
  upid_sz = bpf_core_field_size(pid->numbers[0]);
  bpf_probe_read(&level, sizeof(level), _(&pid->level));
  if (level < 1)
    return 0;
  bpf_probe_read(&upid, upid_sz,
...
FUNC_INLINE __u32 event_find_parent_pid(struct task_struct *t)
{
  ...
  if (!task)
    return 0;
  bpf_probe_read(&pid, sizeof(pid), _(&task->tgid));
  return pid;
}
...
#pragma unroll
  for (i = 0; i < 4; i++) {
    bpf_probe_read(&task, sizeof(task), _(&task->real_parent));
    if (!task)
      break;
    bpf_probe_read(&pid, sizeof(pid), _(&task->tgid));
...
{
  event->key.pid = (bpf_get_current_pid_tgid() >> 32);
