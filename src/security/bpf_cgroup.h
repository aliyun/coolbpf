...
#ifndef SYSAK_BPF_CGROUP_H
#define SYSAK_BPF_CGROUP_H
 ...
 * be read with bpf_probe_read(). NULL on failures.
 */
...
FUNC_INLINE __u32 get_cgroup_level(const struct cgroup *cgrp)
{
  ...
  bpf_probe_read(&level, sizeof(level), _(&cgrp->level));
  return level;
}
                             ...
                             __u32 level)
{
  ...
  heap = bpf_map_lookup_elem(&tg_cgrps_tracking_heap, &zero);
  if (!heap)
    return heap;
...
__init_cgrp_tracking_val_heap(struct cgroup *cgrp, cgroup_state state)
{
  ...
  if (name)
    bpf_probe_read_str(&heap->name, KN_NAME_LENGTH - 1, name);

  return heap;
}

#endif //SYSAK_BPF_CGROUP_H
