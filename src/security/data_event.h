...
#ifndef SYSAK_DATA_EVENT_H
#define SYSAK_DATA_EVENT_H
...
__do_str(void *ctx, struct msg_data *msg, unsigned long arg, bool *done)
{
  ...
  ret = bpf_probe_read_str(&msg->arg[0], max, (char *)arg);
  if (ret < 0)
    return ret;
  ...
  long (*do_data_event)(void *, struct msg_data *, unsigned long, size_t))
{
  ...
  msg = bpf_map_lookup_elem(heap, &zero);
  if (!msg)
    return 0;
