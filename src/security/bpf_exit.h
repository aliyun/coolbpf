...
#ifndef SYSAK_BPF_EXIT_H
#define SYSAK_BPF_EXIT_H
...
FUNC_INLINE void event_exit_send(void *ctx, __u32 tgid)
{
  ...
  if (enter->key.ktime) {
    ...
    exit = bpf_map_lookup_elem(&exit_heap_map, &zero);
    if (!exit)
      return;
    ...
    exit->common.ktime = bpf_ktime_get_ns();
     ...
     */
    exit->info.tid = tgid;
    bpf_probe_read(&exit->info.code, sizeof(exit->info.code),
               _(&task->exit_code));
