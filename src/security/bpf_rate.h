...
#ifndef SYSAK_BPF_RATE_H
#define SYSAK_BPF_RATE_H
...
FUNC_INLINE void send_throttle(void *ctx, struct msg_k8s *kube, __u64 time)
{
  ...
  msg = bpf_map_lookup_elem(&throttle_heap_map, &(__u32){ 0 });
  if (!msg)
    return;
...
FUNC_INLINE bool cgroup_rate(void *ctx, struct msg_k8s *kube, __u64 time)
{
  ...
  opt = bpf_map_lookup_elem(&cgroup_rate_options_map, &zero);
  if (!opt)
    return true;
  ...
  val = bpf_map_lookup_elem(&cgroup_rate_map, &key);
  if (!val) {
    ...
    bpf_map_update_elem(&cgroup_rate_map, &key, &new_value, 0);
    return true;
  }
...
{
  bpf_map_delete_elem(&cgroup_rate_map, &cgroupid);
}


#endif //SYSAK_BPF_RATE_H
