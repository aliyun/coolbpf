...
FUNC_INLINE __u64 __get_auid(struct task_struct *task)
{
  ...
  if (bpf_core_field_exists(task->loginuid)) {
    bpf_probe_read(&auid, sizeof(auid), _(&task->loginuid.val));
  } else {
//    struct audit_task_info *audit;
//
//    if (bpf_core_field_exists(task->audit)) {
//      bpf_probe_read(&audit, sizeof(audit), _(&task->audit));
//      if (audit) {
//        bpf_probe_read(&auid, sizeof(__u32),
//                   _(&audit->loginuid));
//      }
//    }
  }
...
FUNC_INLINE bool hlist_bl_unhashed(const struct hlist_bl_node *h)
{
  ...
  bpf_probe_read(&pprev, sizeof(pprev), _(&h->pprev));
  return !pprev;
}
...
prepend_name(char *buf, char **bufptr, int *buflen, const char *name, u32 namelen)
{
  ...
  *buflen -= (namelen + write_slash);
  ...
  :);
  bpf_probe_read(buf + buffer_offset + write_slash, namelen * sizeof(char), name);
...
FUNC_INLINE long cwd_read(struct cwd_read_data *data)
{
  ...
  bpf_probe_read(&vfsmnt_mnt_root, sizeof(vfsmnt_mnt_root),
             _(&vfsmnt->mnt_root));
  if (dentry == vfsmnt_mnt_root || IS_ROOT(dentry)) {
    struct mount *parent;

    bpf_probe_read(&parent, sizeof(parent), _(&mnt->mnt_parent));
  ...
  bpf_probe_read(&parent, sizeof(parent), _(&dentry->d_parent));
             ...
             char **buffer, int *buflen)
{
  ...
  bpf_probe_read(&data.root_dentry, sizeof(data.root_dentry),
             _(&root->dentry));
  bpf_probe_read(&data.root_mnt, sizeof(data.root_mnt), _(&root->mnt));
  bpf_probe_read(&data.dentry, sizeof(data.dentry), _(&path->dentry));
  bpf_probe_read(&data.vfsmnt, sizeof(data.vfsmnt), _(&path->mnt));
                  ...
                  char **buf, int *buflen)
{
  ...
  bpf_probe_read(&dentry, sizeof(dentry), _(&path->dentry));
...
{
  bpf_probe_read(&msg->effective, sizeof(__u64), _(&cred->cap_effective));
  bpf_probe_read(&msg->inheritable, sizeof(__u64), _(&cred->cap_inheritable));
  bpf_probe_read(&msg->permitted, sizeof(__u64), _(&cred->cap_permitted));
}
...
get_current_subj_caps(struct msg_capabilities *msg, struct task_struct *task)
{
  ...
  bpf_probe_read(&cred, sizeof(cred), _(&task->cred));
  __get_caps(msg, cred);
}
...
get_current_subj_creds(struct msg_cred *info, struct task_struct *task)
{
  ...
  bpf_probe_read(&info->uid, sizeof(__u32), _(&cred->uid));
  bpf_probe_read(&info->gid, sizeof(__u32), _(&cred->gid));
  bpf_probe_read(&info->euid, sizeof(__u32), _(&cred->euid));
  bpf_probe_read(&info->egid, sizeof(__u32), _(&cred->egid));
  bpf_probe_read(&info->suid, sizeof(__u32), _(&cred->suid));
  bpf_probe_read(&info->sgid, sizeof(__u32), _(&cred->sgid));
  bpf_probe_read(&info->fsuid, sizeof(__u32), _(&cred->fsuid));
  bpf_probe_read(&info->fsgid, sizeof(__u32), _(&cred->fsgid));
  bpf_probe_read(&info->securebits, sizeof(__u32), _(&cred->securebits));
...
get_namespaces(struct msg_ns *msg, struct task_struct *task)
{
  ...
  bpf_probe_read(&nsproxy, sizeof(nsproxy), _(&task->nsproxy));
  bpf_probe_read(&nsp, sizeof(nsp), _(nsproxy));

  bpf_probe_read(&msg->uts_inum, sizeof(msg->uts_inum),
             _(&nsp.uts_ns->ns.inum));
  bpf_probe_read(&msg->ipc_inum, sizeof(msg->ipc_inum),
             _(&nsp.ipc_ns->ns.inum));
  bpf_probe_read(&msg->mnt_inum, sizeof(msg->mnt_inum),
             _(&nsp.mnt_ns->ns.inum));
  {
    struct pid *p = 0;

    bpf_probe_read(&p, sizeof(p), _(&task->thread_pid));
    if (p) {
      int level = 0;
      struct upid up;

      bpf_probe_read(&level, sizeof(level), _(&p->level));
      bpf_probe_read(&up, sizeof(up), _(&p->numbers[level]));
      bpf_probe_read(&msg->pid_inum, sizeof(msg->pid_inum),
                 _(&up.ns->ns.inum));
    } else
      msg->pid_inum = 0;
  }
  bpf_probe_read(&msg->pid_for_children_inum,
             sizeof(msg->pid_for_children_inum),
             _(&nsp.pid_ns_for_children->ns.inum));
  bpf_probe_read(&msg->net_inum, sizeof(msg->net_inum),
             _(&nsp.net_ns->ns.inum));

  // this also includes time_ns_for_children
  if (bpf_core_field_exists(nsproxy->time_ns)) {
    bpf_probe_read(&msg->time_inum, sizeof(msg->time_inum),
               _(&nsp.time_ns->ns.inum));
    bpf_probe_read(&msg->time_for_children_inum,
               sizeof(msg->time_for_children_inum),
               _(&nsp.time_ns_for_children->ns.inum));
  }

  bpf_probe_read(&msg->cgroup_inum, sizeof(msg->cgroup_inum),
             _(&nsp.cgroup_ns->ns.inum));
  {
    ...
    bpf_probe_read(&user_ns, sizeof(user_ns), _(&mm->user_ns));
    bpf_probe_read(&msg->user_inum, sizeof(msg->user_inum),
               _(&user_ns->ns.inum));
  }
}
...
__event_get_cgroup_info(struct task_struct *task, struct msg_k8s *kube)
{
  ...
  conf = bpf_map_lookup_elem(&tg_conf_map, &zero);
  if (conf) {
