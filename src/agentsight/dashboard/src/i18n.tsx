...
const enUSMessages = {
  ...
  'nav.reuseLabels': 'Reuse Labels',
  'reuse.title': 'Trajectory Reuse Labels',
  'reuse.intro':
    'Decide which collected trajectories are worth putting in front of a future agent. The deterministic rules propose a label; you settle it. `bad` is deliberately not something the rules may assign on their own — measured on real data they got it wrong every time — so an accusation needs either your judgement or a reviewed model verdict.',
  'reuse.runTriage': 'Run rules',
  'reuse.runJudge': 'Ask the model',
  'reuse.runJudgeHint':
    'Each judgement is a paid request. Off unless the server was configured for it.',
  'reuse.showStats': 'Rule accuracy',
  'reuse.running': 'Running…',
  'reuse.loading': 'Loading…',
  'reuse.empty': 'No labelled trajectories. Run the rules first.',
  'reuse.filterAllLabels': 'All labels',
  'reuse.filterAllStates': 'All states',
  'reuse.summary': '{total} shown · {pending} awaiting a decision',
  'reuse.triageResult':
    'Rules: examined {examined}, labelled {labelled}, unchanged {unchanged} — good {good}, bad {bad}, useless {useless}, unknown {unknown} ({version})',
  'reuse.judgeResult':
    'Model: examined {examined}, judged {judged} — good {good}, bad {bad}, unclear {unclear}, downgraded {downgraded}, failed {failed}',
  'reuse.downgradedNote':
    'A downgraded verdict is one where the model said `bad` but named no supporting step, so it was recorded as unclear instead.',
  'reuse.statsTitle': 'How often each rule was accepted or overturned',
  'reuse.statsEmpty': 'Nothing decided yet, so there is nothing to measure.',
  'reuse.statsRule': 'Rule',
  'reuse.statsConfirmed': 'Accepted',
  'reuse.statsOverridden': 'Overturned',
  'reuse.selectPending': 'Select all awaiting a decision',
  'reuse.confirmSelected': 'Confirm {count} selected',
  'reuse.alreadyDecided': 'Already settled by a person.',
  'reuse.metrics': '{steps} steps · {turns} user turns · {tools} tool calls · {findings} findings',
  'reuse.rulesSaid': 'Rules:',
  'reuse.modelSaid': 'Model:',
  'reuse.modelDowngraded': '(no step cited, recorded as unclear)',
  'reuse.autoChanged':
    'The rules have changed their verdict since this was decided. Your decision still stands; revisit it only if you want to.',
  'reuse.confirm': 'Confirm',
  'reuse.openTrajectory': 'trajectory',
  'reuse.selectBy': 'Select…',
  'reuse.selectByHint': 'Quick-select a group of rows; the checkboxes still allow free picking.',
  'reuse.selectAll': 'All rows',
  'reuse.selectUnknown': 'Unknown, undecided',
  'reuse.selectJudged': 'Already model-judged',
  'reuse.selectNone': 'Clear selection',
  'reuse.runJudgeSelected': 'Ask the model ({count})',
  'reuse.runJudgeSelectedHint':
    'Judge exactly the selected rows — including ones the automatic pick would skip. Each judgement is a paid request.',
  'reuse.rejudge': 'Re-judge',
  'reuse.openTrajectoryHint': 'Open this trajectory in the viewer',
  'reuse.judging': 'Judging…',
  'reuse.rejudgeHint':
    'Ask the model to judge this one trajectory again. Each judgement is a paid request. A human decision still takes precedence.',
  'reuse.untitledHint': 'No user text in this transcript (tool-only). Showing project and date instead.',
  'reuse.subagent': 'sub-agent',
  ...
  'opt.llm.semanticSearchTimeout.label': 'Semantic search timeout (seconds)',
  'opt.llm.semanticSearchTimeout.hint': 'Ranking requests that exceed this deadline return no results and emit a server warning.',
  'opt.llm.semanticSearchTimeout.invalid': 'Enter a positive whole number of seconds.',
  ...
  'comp.settings.storage.title': 'SQLite storage',
  'comp.settings.storage.description': 'Effective retention policies and current on-disk allocation.',
  'comp.settings.storage.loading': 'Loading storage status…',
  'comp.settings.storage.loadFailed': 'Failed to load storage status: {error}',
  'comp.settings.storage.retry': 'Retry',
  'comp.settings.storage.physical': 'Physical size',
  'comp.settings.storage.logical': 'Logical size',
  'comp.settings.storage.retention': 'Retention',
  'comp.settings.storage.limit': 'Size limit',
  'comp.settings.storage.interval': 'Check interval',
  'comp.settings.storage.owner': 'Maintained by',
  'comp.settings.storage.days': '{count} days',
  'comp.settings.storage.inserts': 'Every {count} writes',
  'comp.settings.storage.seconds': 'Every {count} seconds',
  'comp.settings.storage.external': 'External',
  'comp.settings.storage.disabled': 'Disabled',
  'comp.settings.storage.note': 'Logical limits include DB, WAL, and SHM usage minus reusable pages. Cleanup does not shrink the physical database immediately.',
  'comp.settings.storage.store.primary': 'Primary events',
  'comp.settings.storage.store.genai': 'GenAI and evaluations',
  'comp.settings.storage.store.interruptions': 'Interruptions',
  'comp.settings.storage.store.trajectories': 'Trajectories',
  'comp.settings.storage.store.optimization': 'Optimization results',
  'comp.settings.storage.store.reuse': 'Trajectory reuse labels',
  'comp.settings.storage.store.causal': 'Causal attribution',
  'comp.settings.storage.store.security_audit': 'Security audit',
  'comp.settings.storage.store.enforcement': 'Enforcement',
  'comp.settings.storage.store.tokenless': 'Tokenless statistics',
  'comp.settings.storage.state.within_policy': 'Within policy',
  'comp.settings.storage.state.cleanup_due': 'Cleanup due',
  'comp.settings.storage.state.reusable_capacity': 'Reusable capacity',
  'comp.settings.storage.state.disabled': 'Limit disabled',
  'comp.settings.storage.state.unknown': 'Not measured',
  'comp.settings.storage.coverage.full': 'Retention and size managed by AgentSight',
  'comp.settings.storage.coverage.partial': 'Partial cleanup: only data marked safe can be evicted automatically',
  'comp.settings.storage.coverage.row_bounded': 'Bounded by record count',
  'comp.settings.storage.coverage.unmanaged': 'No automatic cleanup in local mode',
  'comp.settings.storage.coverage.external': 'Managed by another component',
  'comp.settings.storage.availability.present': 'Available',
  'comp.settings.storage.availability.missing': 'Not created',
  'comp.settings.storage.availability.error': 'Measurement failed',
  'comp.settings.storage.availability.external': 'Externally managed',
  'comp.settings.storage.maintenance.schedule': 'Maintenance schedule',
  'comp.settings.storage.maintenance.scheduled': 'Scheduled',
  'comp.settings.storage.maintenance.unscheduled': 'Not scheduled',
  'comp.settings.storage.maintenance.worker': 'Worker',
  'comp.settings.storage.maintenance.workerRunning': 'Running',
  'comp.settings.storage.maintenance.workerStopped': 'Stopped',
  'comp.settings.storage.maintenance.heartbeat': 'Worker heartbeat',
  'comp.settings.storage.maintenance.lastResult': 'Latest result',
  'comp.settings.storage.maintenance.lastAttempt': 'Last attempt',
  'comp.settings.storage.maintenance.lastSuccess': 'Last successful attempt',
  'comp.settings.storage.maintenance.failures': 'Consecutive failures',
  'comp.settings.storage.maintenance.nextRun': 'Next check',
  'comp.settings.storage.maintenance.result.success': 'Success',
  'comp.settings.storage.maintenance.result.error': 'Error',
  'comp.settings.storage.maintenance.result.lock_busy': 'Maintenance lock busy',
  'comp.settings.storage.maintenance.result.panicked': 'Worker panicked',
  'comp.settings.storage.maintenance.lockBusyNote': 'Another process may currently hold the maintenance lock.',
  'comp.settings.storage.note.causal': 'Evicted causal results may be recomputed later and incur additional model charges.',
  'comp.settings.storage.note.protected': 'Protected data is not deleted automatically.',
...
export const messages: Record<Locale, Record<MessageKey, string>> = {
  ...
  'zh-CN': {
    ...
    'nav.sessions': '会话列表',
    'nav.reuseLabels': '复用标签',
    'reuse.title': '轨迹复用标签',
    'reuse.intro':
      '判断哪些已收集的轨迹值得拿给后续 Agent 参考。规则先给出一个标签，由你来定案。`bad` 特意不允许规则自行判定——在真实数据上它每次都判错——所以要给出「坏」这个结论，必须由你来判，或者由经过复核的模型来判。',
    'reuse.runTriage': '跑规则判定',
    'reuse.runJudge': '让模型判定',
    'reuse.runJudgeHint': '每次判定都是一次付费请求。服务端未配置时不可用。',
    'reuse.showStats': '规则准确度',
    'reuse.running': '执行中…',
    'reuse.loading': '加载中…',
    'reuse.empty': '还没有已标注的轨迹，先跑一次规则判定。',
    'reuse.filterAllLabels': '全部标签',
    'reuse.filterAllStates': '全部状态',
    'reuse.summary': '共 {total} 条 · {pending} 条待定案',
    'reuse.triageResult':
      '规则：检查 {examined} 条，标注 {labelled} 条，未变 {unchanged} 条 —— 好 {good}，坏 {bad}，无用 {useless}，无法判定 {unknown}（{version}）',
    'reuse.judgeResult':
      '模型：检查 {examined} 条，判定 {judged} 条 —— 好 {good}，坏 {bad}，不确定 {unclear}，降级 {downgraded}，失败 {failed}',
    'reuse.downgradedNote':
      '降级是指模型判了「坏」但没能指出支撑的步骤，因此按「不确定」记录。',
    'reuse.statsTitle': '各规则被采纳与被推翻的次数',
    'reuse.statsEmpty': '还没有人定案，无从统计。',
    'reuse.statsRule': '规则',
    'reuse.statsConfirmed': '采纳',
    'reuse.statsOverridden': '推翻',
    'reuse.selectPending': '全选待定案',
    'reuse.confirmSelected': '确认选中的 {count} 条',
    'reuse.alreadyDecided': '已由人定案。',
    'reuse.metrics': '{steps} 步 · {turns} 轮用户输入 · {tools} 次工具调用 · {findings} 条发现',
    'reuse.rulesSaid': '规则：',
    'reuse.modelSaid': '模型：',
    'reuse.modelDowngraded': '（未指出步骤，按不确定记录）',
    'reuse.autoChanged': '这条定案之后规则改了结论。你的定案依然有效，是否重看由你决定。',
    'reuse.confirm': '确认',
    'reuse.openTrajectory': '轨迹',
    'reuse.selectBy': '选择…',
    'reuse.selectByHint': '按条件快速选中一组行；仍可用复选框自由挑选。',
    'reuse.selectAll': '全部行',
    'reuse.selectUnknown': '未定案的 unknown',
    'reuse.selectJudged': '已被模型判定过',
    'reuse.selectNone': '清除选择',
    'reuse.runJudgeSelected': '让模型判定（{count} 条）',
    'reuse.runJudgeSelectedHint':
      '只判定选中的行——包括自动挑选会跳过的那些。每次判定都是付费请求。',
    'reuse.rejudge': '重判',
    'reuse.openTrajectoryHint': '在轨迹查看器中打开这条轨迹',
    'reuse.judging': '判定中…',
    'reuse.rejudgeHint':
      '让模型对这一条轨迹重新判定。每次判定都是一次付费请求。人工定案的标签仍然优先。',
    'reuse.untitledHint': '该轨迹没有用户文本（纯工具循环），改用项目与日期标识。',
    'reuse.subagent': '子代理',
    ...
    'opt.llm.semanticSearchTimeout.label': '语义搜索超时（秒）',
    'opt.llm.semanticSearchTimeout.hint': '排序请求超过此时限会返回空结果，并在服务端记录警告日志。',
    'opt.llm.semanticSearchTimeout.invalid': '请输入正整数秒数。',
    'opt.llm.saving': '保存中...',
    'opt.llm.save': '保存配置',
    ...
    'comp.settings.storage.title': 'SQLite 存储',
    'comp.settings.storage.description': '查看当前生效的保留策略和磁盘占用。',
    'comp.settings.storage.loading': '正在加载存储状态…',
    'comp.settings.storage.loadFailed': '加载存储状态失败：{error}',
    'comp.settings.storage.retry': '重试',
    'comp.settings.storage.physical': '物理占用',
    'comp.settings.storage.logical': '逻辑占用',
    'comp.settings.storage.retention': '保留时间',
    'comp.settings.storage.limit': '容量上限',
    'comp.settings.storage.interval': '检查间隔',
    'comp.settings.storage.owner': '执行进程',
    'comp.settings.storage.days': '{count} 天',
    'comp.settings.storage.inserts': '每 {count} 次写入',
    'comp.settings.storage.seconds': '每 {count} 秒',
    'comp.settings.storage.external': '外部管理',
    'comp.settings.storage.disabled': '已关闭',
    'comp.settings.storage.note': '逻辑上限统计 DB、WAL、SHM 并扣除可复用页面。清理不会立即缩小数据库物理文件。',
    'comp.settings.storage.store.primary': '主事件库',
    'comp.settings.storage.store.genai': 'GenAI 与评估',
    'comp.settings.storage.store.interruptions': '中断事件',
    'comp.settings.storage.store.trajectories': '轨迹',
    'comp.settings.storage.store.optimization': '优化结果',
    'comp.settings.storage.store.reuse': '轨迹复用标签',
    'comp.settings.storage.store.causal': '因果归因',
    'comp.settings.storage.store.security_audit': '安全审计',
    'comp.settings.storage.store.enforcement': '风险处置',
    'comp.settings.storage.store.tokenless': 'Tokenless 统计',
    'comp.settings.storage.state.within_policy': '符合策略',
    'comp.settings.storage.state.cleanup_due': '等待清理',
    'comp.settings.storage.state.reusable_capacity': '存在可复用空间',
    'comp.settings.storage.state.disabled': '容量限制已关闭',
    'comp.settings.storage.state.unknown': '未测量',
    'comp.settings.storage.coverage.full': '由 AgentSight 管理保留时间与容量',
    'comp.settings.storage.coverage.partial': '部分自动清理：仅标记为安全的数据可被自动淘汰',
    'comp.settings.storage.coverage.row_bounded': '按记录数量限制',
    'comp.settings.storage.coverage.unmanaged': '本地模式未启用自动清理',
    'comp.settings.storage.coverage.external': '由其他组件管理',
    'comp.settings.storage.availability.present': '可用',
    'comp.settings.storage.availability.missing': '尚未创建',
    'comp.settings.storage.availability.error': '测量失败',
    'comp.settings.storage.availability.external': '外部管理',
    'comp.settings.storage.maintenance.schedule': '维护调度',
    'comp.settings.storage.maintenance.scheduled': '已调度',
    'comp.settings.storage.maintenance.unscheduled': '未调度',
    'comp.settings.storage.maintenance.worker': '维护 worker',
    'comp.settings.storage.maintenance.workerRunning': '运行中',
    'comp.settings.storage.maintenance.workerStopped': '已停止',
    'comp.settings.storage.maintenance.heartbeat': 'Worker 心跳',
    'comp.settings.storage.maintenance.lastResult': '最近结果',
    'comp.settings.storage.maintenance.lastAttempt': '最近尝试',
    'comp.settings.storage.maintenance.lastSuccess': '最近成功尝试',
    'comp.settings.storage.maintenance.failures': '连续失败次数',
    'comp.settings.storage.maintenance.nextRun': '下次检查',
    'comp.settings.storage.maintenance.result.success': '成功',
    'comp.settings.storage.maintenance.result.error': '错误',
    'comp.settings.storage.maintenance.result.lock_busy': '维护锁忙',
    'comp.settings.storage.maintenance.result.panicked': 'Worker 已 panic',
    'comp.settings.storage.maintenance.lockBusyNote': '另一进程可能正在持有维护锁。',
    'comp.settings.storage.note.causal': '被淘汰的因果结果后续可能重新计算，并产生额外的模型费用。',
    'comp.settings.storage.note.protected': '受保护数据不会被自动删除。',
