...
impl SecurityStore {
    ...
    pub fn open_default() -> Result<Self, SecurityStoreError> {
        Self::open_private(crate::config::default_base_path())
    }
    ...
    /// Opens production state with a private directory, database, and WAL sidecars.
    pub(crate) fn open_private(state_dir: impl AsRef<Path>) -> Result<Self, SecurityStoreError> {
        let conn =
    ...
    pub fn summary(&self) -> Result<SecuritySummary, SecurityStoreError> {
        self.summary_filtered(&SecurityEventFilter::default())
    }
    ...
    pub fn summary_filtered(
        &self,
        filter: &SecurityEventFilter,
    ) -> Result<SecuritySummary, SecurityStoreError> {
    ...
    pub fn list_sessions(
        &self,
        filter: &SecurityEventFilter,
    ) -> Result<SecuritySessionPage, SecurityStoreError> {
    ...
    ///
    /// Returns a typed database, stored-data, or lock error.
    pub fn case_count(&self) -> Result<u64, SecurityStoreError> {
        let count: i64 =
            self.connection()?
...
fn open_connection(path: &Path) -> Result<Connection, SecurityStoreError> {
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent).map_err(|error| {
            SecurityStoreError::Open(format!("create database directory {parent:?}: {error}"))
        })?;
    }
    let conn = Connection::open(path)
        .map_err(|error| SecurityStoreError::Open(format!("open SQLite {path:?}: {error}")))?;
