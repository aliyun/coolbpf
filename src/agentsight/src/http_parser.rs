...
impl HTTPParser {
    ...
    /// Returns an Event if the SSL data contains valid HTTP, None otherwise
    pub fn handle_ssl_event(event: &SslEvent, include_raw_data: bool) -> Option<HTTPEvent> {
        ...
        if Self::is_http_data(data_str) {
            if let Some(parsed_message) = Self::parse_http_message(data_str) {
                let tid = event.tid as u64;
                return Some(Self::create_http_event(
                    tid,
                    parsed_message,
                    include_raw_data,
                ));
