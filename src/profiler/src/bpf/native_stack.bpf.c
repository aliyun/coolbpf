...
#define STACK_DELTA_BUCKET(X) \
  BPF_MAP(exe_id_to_##X##_stack_deltas, BPF_MAP_TYPE_HASH_OF_MAPS, u64, u32, 4096)
// struct inner_map_##X##_stack_deltas                     \
  // {                                                       \
  //   __uint(type, BPF_MAP_TYPE_ARRAY);                     \
  //   __uint(max_entries, 1 << X);                          \
  //   __type(key, u32);                                     \
  //   __type(value, StackDelta);                            \
  // } inner_map_##X##_stack_deltas SEC(".maps");            \
  // struct exe_id_to_##X##_stack_deltas                     \
  // {                                                       \
  //   __uint(type, BPF_MAP_TYPE_HASH_OF_MAPS);              \
  //   __uint(max_entries, 4096);                            \
  //   __uint(key_size, sizeof(u32));                        \
  //   __array(values, struct inner_map_##X##_stack_deltas); \
  // } exe_id_to_##X##_stack_deltas SEC(".maps") = {         \
  //     .values = {                                         \
  //         [0] = &inner_map_##X##_stack_deltas,            \
  //     }};
...
static inline __attribute__((__always_inline__)) bool bsearch_step(void *inner_map, u32 *lo, u32 *hi, u16 page_offset)
{
  u32 pivot = (*lo + *hi) >> 1;
