...
static inline void event_send_trigger(struct pt_regs *ctx, u32 event_type) {
  ...
  // BUG: 不能按照otel的写法，编译可能会出现问题
  ret = bpf_map_update_elem(&inhibit_events, &inhibit_key, &inhibit_value, BPF_NOEXIST);
  if (ret < 0) {
...
// extraction round.
static inline __attribute__((__always_inline__)) bool unwinder_is_done(const PerCPURecord *record, int unwinder)
{
...
static inline __attribute__((__always_inline__)) void unwinder_mark_done(PerCPURecord *record, int unwinder)
{
...
static inline __attribute__((__always_inline__))
ErrorCode
_push_with_max_frames(Trace *trace, u64 file, u64 line, u8 frame_type, u8 return_address, u32 max_frames)
{
...
static inline __attribute__((__always_inline__))
ErrorCode
push_error(Trace *trace, ErrorCode error)
{
...
{
  u64 elem_sz = sizeof(Frame);
  if (trace->user_stack_id == 0x7fffffff)
  {
    elem_sz = sizeof(u64);
  }
  const u64 send_size = offsetof(Trace, frames) + elem_sz * trace->stack_len;
...
static inline __attribute__((__always_inline__)) int get_next_unwinder_after_interpreter(const PerCPURecord *record)
{
