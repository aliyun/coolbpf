...
int j_sock_release(struct pt_regs *ctx)
{
    ...
    if (BPF_CORE_READ(sk, sk_wmem_alloc.refs.counter)) {
        return 0;
    }
    u64 k = (u64)sk;
...
#define SKB_CB(__skb)       BPF_CORE_READ(__skb, cb[0])
SEC("raw_tracepoint/tcp_retransmit_skb")
int e_tcp_retransmit_skb_hook(struct bpf_raw_tracepoint_args *ctx){
    struct sock *sk;
    struct sk_buff *skb;
    char cb;
    struct tcp_skb_cb *skb_cb;
    u8 flag;
    struct data_t data = {};

    skb = (struct sk_buff *)ctx->args[1];
    cb = SKB_CB(skb);
    skb_cb = (struct tcp_skb_cb *)&cb;
    flag = BPF_CORE_READ(skb_cb, tcp_flags);
    if (flag & TCPHDR_SYN)
    {
        sk  = (struct sock *)ctx->args[0];
        get_info(&data, sk, 5);
        get_task(&data, sk);
        bpf_perf_event_output(ctx, &net_map, BPF_F_CURRENT_CPU, &data, sizeof(data));
    }
...
SEC("raw_tracepoint/tcp_retransmit_synack")
int e_tcp_retransmit_synack(struct bpf_raw_tracepoint_args *ctx)
{
    struct sock *sk;
    struct data_t data = {};
